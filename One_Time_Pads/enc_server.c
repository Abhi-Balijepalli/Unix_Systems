...
void encryption(char message[],char key[], int sLen);
...
/*
 * Description: bruh why are u reading encription for the main()
 */
int main(int argc, char *argv[]){
    int socketFD;                   //Same variables from enc_client.c
    int portNumber; 
    int status;                
    ...
    //this is straight from the enc_client.c
    if (argc != 2) {
    ...
    while(1){
        ...
        if(pid == 0){
            ...
            encryption(message,keyS,strlen(message));
...
/*
 * encription: Don't judge, this way to change from char to int was easier.
 */
int charToInt(char c){
...
/*
 * encription: encrpts the server side of things
 */
void encryption(char message[],char key[], int sLen){
    int i = 0;
    int fNum,keyNum, encNum;
    sLen = strlen(message)-1;
    while(i < sLen){
        fNum = charToInt(message[i]);
        keyNum = charToInt(key[i]);
        encNum = (fNum + keyNum) % 27;
        message[i]=  IntToChar(encNum);
        i++;
    }
    message[i] = '\0';
    return;
